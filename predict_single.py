...
class Predictor:
    ...
    def old_predict(self):
        args, eval_task, tokenizer, model = self.args, self.eval_task, self.tokenizer, self.model
        eval_dataset = run_glue_gap.load_and_cache_examples(args, eval_task, tokenizer, evaluate=True)

    def predict(self,stringa):
        args, eval_task, tokenizer, model = self.args, self.eval_task, self.tokenizer, self.model
        eval_dataset = run_glue_gap.load_single_example(args, eval_task, tokenizer, stringa, evaluate=True)
